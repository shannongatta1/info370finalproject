...
We used the same 10 features with the KNN model because we wanted to compare the accuracy scores of the two models. Before doing any cross validation or grid research, we were able to accurately predict 74% of the outcome. Then we ran a 10-fold cross validation and the mean score of 71%. By using grid search, we found the best parameters of the RFC which the criterion was entropy, max depth was 4, max features were auto and the number of estimators was 500. After applying the trained model to our dataset, we were able to accurately predict 76% of the outcome.

### Comparison and Visualization

The following two graphs depicted are comparing the number of accurate and inaccurate matches of the machine learning algorithm used to predict and the outcomes. Both algorithms show a consistent pattern of predicting the candidates who lost the primary, with Random Forest Regression at 76% and K Nearest Neighbors at 73%.
